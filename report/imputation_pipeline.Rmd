...
- AT/CG genotypes are deleted from GWAS files (this was done for Detroit before we received the GWAS files, and we need to be consistent); for ADPC files that strategy would result in the deletion of about 70 000 SNPs, so for these ADPC SNPs: SNPs with MAF >= 0.3 are deleted, SNPs with MAF < 0.3 that have a different minor allele as the CAAPA reference SNP: if the CAAPA reference SNP also has MAF < 0.3, the strand of the SNP is flipped, else the SNP is deleted 
