...
site=$1
in_file_prefix=../data/working/${site}/gwas_qc
snp_file_name=../data/working/${site}/gwas_AC_GT.txt
no_at_cg_file_prefix=../data/working/${site}/gwas_no_AT_CG
vcf_file_prefix=../data/working/${site}/gwas_chr
out_file_prefix=../data/working/${site}/gwas_flipped
...
echo "(name the files gwas.pdf and gwas_statistics.txt)"
echo "then press ENTER"
echo "================================================================================="
read

#Get the list of SNPs to flip
stats_file=../data/output/${site}/statistics/gwas_statistics.txt
flip_file=../data/working/${site}/gwas_flip.txt
...
echo "m_ambiguous_gwas $m_ambig" >> ../data/output/${site}/flow/flow_nrs.txt
m_flip=`wc -l $flip_file | tr -s ' ' | cut -f2 -d' '`
echo "m_flip_gwas $m_flip" >> ../data/output/${site}/flow/flow_nrs.txt
n_stranded=`wc -l ${out_file_prefix}.fam | tr -s ' ' | cut -f2 -d' '`
echo "n_stranded_gwas $n_stranded" >> ../data/output/${site}/flow/flow_nrs.txt
m_stranded=`wc -l ${out_file_prefix}.bim | tr -s ' ' | cut -f2 -d' '`
echo "m_stranded_gwas $m_stranded" >> ../data/output/${site}/flow/flow_nrs.txt
