...
out.file <- args[5] #precision_recall.txt
...
for (chr in 1:22) {
  ...
  chr.snp.info <- chr.snp.info[chr.snp.info$MAF < 0.05,]
...
imputed.snps <- paste(imputed.frame$CHROM, imputed.frame$POS, sep=":")
imputed.frame <- imputed.frame[!duplicated(imputed.snps),]
cat("SNP\tMAF\tRsq\tprecision\trecall\n", file=out.file)
if (identical(typed.frame$POS, imputed.frame$POS)) {
  ...
  for (r in 1:dim(typed.frame)[1]) {
    ...
    if ((snp %in% snp.info$SNP) & !(is.na(typed.row[1,1]))) {
      imputed.row <- imputed.frame[r,cols]
      maf <- snp.info$MAF[snp.info$SNP == snp]
      rsq <- snp.info$Rsq[snp.info$SNP == snp]
      typed.rare <- rep(0, n)
      imputed.rare <- rep(0, n)
      if (sum(typed.row == "0/0") < sum(typed.row == "1/1")) {
        typed.rare[typed.row == "0/0"] <- 1
        imputed.rare[imputed.row == "0/0"] <- 1
      } else {
        typed.rare[typed.row == "1/1"] <- 1
        imputed.rare[imputed.row == "1/1"] <- 1  
      }
      typed.rare[typed.row == "0/1"] <- 1
      imputed.rare[imputed.row == "0/1"] <- 1
      true.pos <- sum((typed.rare == 1) & (imputed.rare == 1))
      false.pos <- sum((typed.rare == 0) & (imputed.rare == 1))
      false.neg <- sum((typed.rare == 1) & (imputed.rare == 0))
      precision <- true.pos/(true.pos + false.pos)
      recall <- true.pos/(true.pos + false.neg)
      cat(snp, maf, rsq, precision, paste0(recall, "\n"), sep="\t", file=out.file, append = T)
    }
  }
